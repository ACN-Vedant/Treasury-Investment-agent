# Step 3 — Ratings Sub-Agent

**Phase:** Identify Opportunities for Investment  
**Runs:** Automatically (no human decision needed)  
**Time:** ~1 minute  
**Input:** Sheet 3 (Investment Positions) + Sheet 4 (Market Rates & Policy)  
...
> **Note:** This agent runs in parallel with the Compliance Sub-Agent (Step 2). You can run Steps 2 and 3 simultaneously in two separate browser tabs if you want to save time.
...
Go to [claude.ai](https://claude.ai) and click **New chat**. This must be a fresh conversation — do not continue in the Step 2 chat.
...
```
INVESTMENT POSITIONS:
Position_ID	Product_Type	Counterparty	Notional_Amount	Currency	Start_Date	Maturity_Date	Tenor_Days	Yield_Percent	Credit_Rating	Max_Counterparty_Limit	Current_Exposure	Headroom	TMS_Reference
POS-001	Fixed Deposit	Barclays	10000000	USD	01/03/2026	01/09/2026	184	5.10%	AA+	20000000	10000000	10000000	TMS-2026-0031
POS-002	T-Bill	JPMorgan	5000000	USD	01/04/2026	01/07/2026	91	5.05%	AAA	25000000	5000000	20000000	TMS-2026-0032
POS-003	Fixed Deposit	HSBC	8000000	GBP	15/03/2026	15/09/2026	184	4.95%	AA-	20000000	8000000	12000000	TMS-2026-0033
...
...
> **Tip:** You only need to paste the `Min_Credit_Rating` row from Sheet 4 for this agent — you do not need to paste the entire policy sheet. However, pasting the full sheet is also fine.
...
```
=== STEP 3 OUTPUT — RATINGS SUB-AGENT ===
[paste JSON here]
```
...
| Problem | Fix |
|---------|-----|
| Numeric scores look incorrect | Check the rating column in Sheet 3 — common issues are spaces (e.g. "AA +" instead of "AA+") or lowercase letters |
| `counterparties_below_minimum` is unexpectedly long | Verify the `Min_Credit_Rating` value in Sheet 4 — if it was recently updated, some counterparties may now be excluded |
...
*Source: Treasury Investment AI Agent Implementation Guide — Section 4.3 (Ratings Sub-Agent)*
